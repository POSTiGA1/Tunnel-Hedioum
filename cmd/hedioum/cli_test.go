...
func TestExpandMimics(t *testing.T) {
	if got, _ := expandMimics("all"); len(got) != len(mimicTypesAll) || got[0] != "ssh" {
		t.Fatalf("all => %v (want the whole arsenal starting with ssh)", got)
	}
	...
	// Implicit-TLS, STARTTLS mail/db, panel and registry mimics are all valid types.
	for _, m := range []string{"smtp", "imap", "smtps", "imaps", "directadmin", "https-alt", "docker", "postgres", "mysql"} {
		if got, err := expandMimics(m); err != nil || len(got) != 1 || got[0] != m {
			t.Fatalf("%s => %v, %v", m, got, err)
		}
	}
	if got, _ := expandMimics("ssh,tls,smtp,imap,smtps,imaps,directadmin,docker,postgres,mysql,https-alt"); len(got) != 11 {
