...
func clientMimicFor(ep config.Endpoint, token string) mimic.ClientMimic {
	...
	case "tls", "smtps", "imaps", "directadmin", "https-alt", "docker", "grafana", "prometheus":
		// Implicit TLS: smtps/imaps/directadmin/https-alt/docker/grafana/prometheus all
		// share the plain TLS mimic on different ports (the disguise is server-side: the
		// panel/registry decoy shown to unauthorized probes).
